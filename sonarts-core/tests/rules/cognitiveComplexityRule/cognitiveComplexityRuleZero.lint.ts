...
//^^^^^^^^^^^ {{Refactor this constructor to reduce its Cognitive Complexity from 1 to the 0 allowed.}}
    if (condition) { }
  }

  get field(): string {
//    ^^^^^ {{Refactor this accessor to reduce its Cognitive Complexity from 1 to the 0 allowed.}}
    if (this._field) {
      return this._field;
    }
    return "";
