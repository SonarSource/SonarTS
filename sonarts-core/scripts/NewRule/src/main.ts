...
function showHelp() {
    ...
    console.log("java -jar /tools/rule-api-1.17.0.1017.jar generate -language ts -rule S4043 -preserve-filenames -no-language-in-filenames ")
    console.log("")
    console.log("Usage:")
...
function createFiles(rootFolder: string, ruleClassName: string, ruleNameDash: string, ruleTitle: string, rspecKey: string, isTypescriptOnly: boolean) {
    ...
    copyWithReplace(`${templatesFolder}/rule_template.ts`,
    ...
    copyWithReplace(`${templatesFolder}/testCase_template.ts`,
        `${testPath}/${ruleClassName}.test.ts`,
        dict)

    copyWithReplace(`${templatesFolder}/unitTest_template.ts`,
        `${testPath}/${ruleClassName}.lint.ts`,
        dict)
}
